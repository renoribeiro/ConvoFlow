...
export const FEATURE_HELP: Record<string, FeatureHelpEntry> = {
  ...
  'page:conversations': {
    ...
    tips: [
      ...
      'Resposta dada pelo app do Instagram no celular também aparece aqui, sem autor. A que você manda por esta tela aparece uma vez só, com o seu nome. Ela ainda não tira a conversa de "Aguardando": abra a conversa aqui para zerar as não lidas.',
  ...
  'page:whatsapp-numbers': {
    ...
    howToConfigure: [
      ...
      'Se o cartão do Instagram mudar, ele diz o que houve. "Vence em", seguido de quantos dias faltam: a renovação automática não conseguiu renovar e tenta de novo no dia seguinte. "Reconectar": o Instagram não aceita mais renovar aquele acesso, por exemplo porque a senha da conta mudou ou a permissão do ConvoFlow foi retirada no Instagram. "Vencida": passou da data, e as respostas pelo Instagram param. Nos dois últimos casos só reconectar a conta resolve. Por enquanto essa reconexão é feita pela equipe do ConvoFlow: escreva para contato@convoflow.com.br.',
      'Na Loja com Instagram, a tela tem duas seções: "Instâncias WhatsApp" e "Contas do Instagram". O cartão de cada conta mostra o @, até quando a conexão vale e o estado dela. Os contadores do topo somam as duas seções: o primeiro vira "Total de conexões" e diz quantas são de cada canal, e uma conta do Instagram vencida ou por reconectar conta como desconectada.',
      'Conectar uma conta do Instagram ainda não se faz por esta tela: quem conecta é a equipe do ConvoFlow (escreva para contato@convoflow.com.br). No cartão da conta você usa o lápis para renomear e a lixeira para excluir. Gerente e Gestor veem os dois botões; Atendente, nenhum.',
    ],
    example:
  ...
  'page:admin-billing': {
    ...
    whatItDoes:
      'É o dinheiro da plataforma, lido do Stripe: quantas assinaturas estão ativas, a receita mensal, o total faturado e as transações. É também onde os cupons de desconto são criados. Arquivar um cupom é para sempre.',
    howToConfigure: [
