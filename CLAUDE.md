...
- `20260914000001_conversation_visibility.sql` — the **SELECT** policies of `conversations` and `messages` (per-atendente visibility via `conversation_visibility_level()` + `conversation_participants`); `messages`' old `ALL` policy became four. Writes stay tenant-scoped, but read the header before touching either table: Postgres also applies SELECT policies to `UPDATE … WHERE` and to the UPDATE path of `ON CONFLICT`, which is why `handle_message_conversation` and `update_conversation_on_message` are `SECURITY DEFINER` now. Safety net: `docs/teste_visibilidade_conversas.sql` (plus the two older suites).
- `20260915000001_conversation_rotation.sql` — automatic distribution of new conversations (`conversation_rotation` table, read-only for gestor/gerente; writes only via `set_conversation_rotation` and triggers). Touches **no** policy of `conversations`/`messages`. Two things to know before adding triggers to `messages`: AFTER triggers fire in **name order**, and the picker is deliberately named `zz_rotation_assign_on_inbound` so it fires last (the conferência block aborts if something sorts after it); and every trigger on the inbound path must be `EXCEPTION WHEN OTHERS → RAISE WARNING → RETURN NEW`, because on the Evolution path the webhook discards INSERT errors and a raised trigger silently loses the customer's message. Safety nets: `docs/teste_rotacao_conversas.sql` and `docs/teste_rotacao_concorrencia.sql` (two-session proof).
