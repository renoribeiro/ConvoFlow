...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const fullPath = `/message/${apiPath}`;
  
  try {
    console.log(`🚀 [message-proxy] Proxy para Evolution API - ${req.method} ${fullPath}`);
    console.log('📤 [message-proxy] Payload:', req.method !== 'GET' ? JSON.stringify(req.body, null, 2) : 'N/A');
    ...
    console.log('📥 [message-proxy] Resposta da Evolution API:', {
    ...
    console.error(`❌ [message-proxy] Erro no proxy da Evolution API para ${fullPath}:`, error);
